...
# This nodepol is created when the dedicated-ui or advanced logscale_cluster_type is defined
resource "google_container_node_pool" "logscale_ui_node_pool" {
  ...
  name       = (var.logscale_gke_cluster_name != "" ? "${var.logscale_gke_cluster_name}-np-ui" : "${var.infrastructure_prefix}-np-ui")
  location   = var.region
