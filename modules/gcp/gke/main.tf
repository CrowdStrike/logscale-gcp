...
resource "google_container_cluster" "logscale" {
  name     = (var.logscale_gke_cluster_name != "" ? var.logscale_gke_cluster_name : "${var.infrastructure_prefix}-gke")
  location = var.region
  ...
  master_authorized_networks_config {
    gcp_public_cidrs_access_enabled = var.kubernetes_private_cluster_enabled == true ? false : true
    dynamic "cidr_blocks" {
      for_each = var.ip_ranges_allowed_to_kubeapi
      content {
  ...
  private_cluster_config {
    ...
    enable_private_endpoint = var.kubernetes_private_cluster_enabled
  }
