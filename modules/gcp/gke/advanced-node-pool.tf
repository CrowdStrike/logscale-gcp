...
resource "google_container_node_pool" "logscale_ingest_node_pool" {
  ...
  name       = (var.logscale_gke_cluster_name != "" ? "${var.logscale_gke_cluster_name}-np-ingest-${random_string.node_pool_suffix.result}" : "${var.infrastructure_prefix}-${var.env_identifier_rand}-np-ingest-${random_string.node_pool_suffix.result}")
  location   = var.region
  ...
  node_config {
    ...
    labels = {
      ...
      k8s-app    = "logscale-ingest-${var.env_identifier_rand}"
    }
...
resource "google_container_node_pool" "logscale_ui_node_pool" {
  count      = contains(["internal-ingest"], var.logscale_cluster_type) ? 1 : 0
  provider   = google-beta
  name       = (var.logscale_gke_cluster_name != "" ? "${var.logscale_gke_cluster_name}-np-ui-${random_string.node_pool_suffix.result}" : "${var.infrastructure_prefix}-${var.env_identifier_rand}-np-ui-${random_string.node_pool_suffix.result}")
  location   = var.region
  cluster    = google_container_cluster.logscale.name
  ...
  node_config {
    ...
    labels = {
      managed_by = "terraform"
      k8s-app    = "logscale-ui-${var.env_identifier_rand}"
    }
